	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="用户信息" prop="user_information">
													<el-select v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','user_information')) || (!form['complaint_information_id'] && $check_field('add','user_information'))" id="user_information" v-model="form['user_information']" :disabled="disabledObj['user_information_isDisabled']">
				...
				<el-form-item label="用户姓名" prop="user_name">
							  ...
							  v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','user_name')) || (!form['complaint_information_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']"></el-input>
				...
				<el-form-item label="用户性别" prop="user_gender">
						...
						v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','user_gender')) || (!form['complaint_information_id'] && $check_field('add','user_gender'))">
				...
				<el-form-item label="用户年龄" prop="user_age">
							  ...
							  v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','user_age')) || (!form['complaint_information_id'] && $check_field('add','user_age'))" :disabled="disabledObj['user_age_isDisabled']"></el-input>
				...
				<el-form-item label="咨询师信息" prop="consultant_information">
													<el-select v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','consultant_information')) || (!form['complaint_information_id'] && $check_field('add','consultant_information'))" id="consultant_information" v-model="form['consultant_information']" :disabled="disabledObj['consultant_information_isDisabled']">
				...
				<el-form-item label="咨询师姓名" prop="name_of_consultant">
							  ...
							  v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','name_of_consultant')) || (!form['complaint_information_id'] && $check_field('add','name_of_consultant'))" :disabled="disabledObj['name_of_consultant_isDisabled']"></el-input>
				...
				<el-form-item label="预约时间" prop="time_of_appointment">
								<el-date-picker :disabled="disabledObj['time_of_appointment_isDisabled']" v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','time_of_appointment')) || (!form['complaint_information_id'] && $check_field('add','time_of_appointment'))" id="time_of_appointment"
					...
					<div v-else-if="$check_field('get','time_of_appointment')">{{form['time_of_appointment']}}</div>
								...
								<el-col v-if="user_group === '管理员' || $check_field('get','reasons_for_complaint') || $check_field('add','reasons_for_complaint') || $check_field('set','reasons_for_complaint')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="投诉原因" prop="reasons_for_complaint">
								<el-input type="textarea" id="reasons_for_complaint" v-model="form['reasons_for_complaint']" placeholder="请输入投诉原因"
						v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','reasons_for_complaint')) || (!form['complaint_information_id'] && $check_field('add','reasons_for_complaint'))" :disabled="disabledObj['reasons_for_complaint_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','reasons_for_complaint')">{{form['reasons_for_complaint']}}</div>
							</el-form-item>
			</el-col>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "complaint_information_id",
				url_add: "~/api/complaint_information/add?",
				url_set: "~/api/complaint_information/set?",
				url_get_obj: "~/api/complaint_information/get_obj?",
				url_upload: "~/api/complaint_information/upload?",

				query: {
					"complaint_information_id": 0,
				},
				...
				form: {
								"user_information": 0, // 用户信息
										...
										"reasons_for_complaint":  '', // 投诉原因
											"complaint_information_id": 0, // ID
				...
				disabledObj:{
								"user_information_isDisabled": false,
										...
										"reasons_for_complaint_isDisabled": false,
										},
		...
		methods: {
          ...
          this.form["time_of_appointment"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/complaint_information/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/complaint_information/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/complaint_information/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/complaint_information/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/complaint_information/view','get');
