<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="心理咨询推荐"
      url="/pages/psychological_counseling/list"
      :list="list_article"
      v-if="$check_action('/psychological_counseling/list', 'get')"
    >
	  <list_psychological_counseling :list="list_psychological_counseling"></list_psychological_counseling>
...
<script>
				...
				import list_psychological_counseling from "@/components/diy/list_psychological_counseling.vue";
...
export default {
  ...
  methods: {
    ...
    toToggle() {
      this.isAdmin = !this.isAdmin;
		...
		*  获取心理咨询
		*/
		get_psychological_counseling() {
			let url = "~/api/psychological_counseling/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("心理咨询" ,json.result.list);
					this.list_psychological_counseling = json.result.list;
											this.list_psychological_counseling.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_psychological_counseling ,"psychological_counseling" ,"15046");
									}
			});
		},
    ...
    get_menu() {
      ...
      this.$get(
        ...
        (json) => {
          ...
          this.list_menu.unshift({ mod_name: "心理测试", path: "/exam/list" });
        }
      );
    },
  ...
  onShow() {
    						this.get_psychological_counseling();
				    this.get_menu();
    this.get_slides();
			this.get_article();
    this.get_link();
