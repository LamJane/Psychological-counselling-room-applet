<template>
	<view class="page_psychological_counseling diy_list diy_list_page div_list_tml" id="psychological_counseling_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
														<uni-search-bar placeholder="搜索咨询师姓名" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'name_of_consultant')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																				<uni-search-bar placeholder="搜索专业主治" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'professional_treatment')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																			<uni-search-bar placeholder="搜索擅长领域" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'areas_of_expertise')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
															<!-- /搜索栏 -->
						</view>
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?psychological_counseling_id=' + o['psychological_counseling_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
			...
			return {
				url_get_list: "~/api/psychological_counseling/get_list?like=0",
					...
					list_user_consultant_information: [],
										// 咨询师性别选项列表
				list_gender_of_consultant: [{value:"全部",text:"全部"}],
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=psychological_counseling", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["psychological_counseling_id"] === o["source_id"]){
