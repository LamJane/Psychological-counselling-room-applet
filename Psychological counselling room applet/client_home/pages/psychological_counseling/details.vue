<template>
	<view class="page_psychological_counseling diy_detail diy_detail--new" id="psychological_counseling_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					<view v-if="$check_field('get','consultant_information')" class="col-12 col-6">
						<view class="view">
					...
					<view v-if="$check_field('get','name_of_consultant')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>咨询师姓名</span>
							</view>
					...
					<view v-if="$check_field('get','gender_of_consultant')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>咨询师性别</span>
							</view>
					...
					<view v-if="$check_field('get','professional_treatment')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>专业主治</span>
							</view>
					...
					<view v-if="$check_field('get','areas_of_expertise')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>擅长领域</span>
							</view>
					...
					<view v-if="$check_field('get','introduction_to_consultants')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>咨询师简介</span>
							</view>
					...
					<view v-if="$check_field('get','photos_of_consultants')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>咨询师照片</span>
							</view>
								<view class="diy_field diy_img">
								<img :src="$fullUrl(obj['photos_of_consultants'])" />
							</view>
							</view>
					</view>
				</view>
		...
		<view class="warp">
			<view class="container-fluid container-fluid-comment--new">
				<view class="row">
					<view class="col">
									<button type="button" class="btn btn-primary" v-if="$check_action('/appointment_information/edit','add')" @click="to_form('/pages/appointment_information/edit')">
							<span>在线预约</span>
						</button>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=psychological_counseling&source_field=psychological_counseling_id&source_id=' + obj['psychological_counseling_id']">
							评论
						</navigator>
	...
	export default {
			...
			return {
				...
				obj: {
						"consultant_information": 0,
							...
							"areas_of_expertise": "",
							"introduction_to_consultants": "",
							"photos_of_consultants": "",
						"hits": 0,
					"praise_len": 0,
					"psychological_counseling_id": 0,
		...
		methods: {
			...
			get_praise(obj) {
				...
				this.$get("~/api/praise/count?", {
					source_table: "psychological_counseling",
					source_field: "psychological_counseling_id",
					source_id: obj["psychological_counseling_id"]
				}, (res) => {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "psychological_counseling",
					source_field: "psychological_counseling_id",
					source_id: this.obj["psychological_counseling_id"],
					user_id
				};
        ...
        _this.obj.praise_len = parseInt(_this.obj.praise_len)
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/psychological_counseling/set?psychological_counseling_id=' + _this.obj["psychological_counseling_id"], {
								praise_len
							}, (res) => {
								if(res.result){
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					this.state_collect = true;
													query.title = this.obj.name_of_consultant
																																		query.img = this.obj.photos_of_consultants
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/psychological_counseling/set?psychological_counseling_id=' + obj["psychological_counseling_id"], {
					hits
				}, (res) => {
					if(res.result){
