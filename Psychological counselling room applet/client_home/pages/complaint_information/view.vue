          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user_information') || $check_field('add','user_information') || $check_field('set','user_information')" label="用户信息" name="user_information">
                  ...
                  v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','user_information')) || (!form['complaint_information_id'] && $check_field('add','user_information'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['user_information']"
                  :localdata="list_user_user_information"
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" label="用户姓名" name="user_name">
                                <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','user_name')) || (!form['complaint_information_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_age') || $check_field('add','user_age') || $check_field('set','user_age')" label="用户年龄" name="user_age">
                                <uni-easyinput type="text" v-model="form['user_age']" v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','user_age')) || (!form['complaint_information_id'] && $check_field('add','user_age'))" :disabled="disabledObj['user_age_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','consultant_information') || $check_field('add','consultant_information') || $check_field('set','consultant_information')" label="咨询师信息" name="consultant_information">
                  ...
                  v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','consultant_information')) || (!form['complaint_information_id'] && $check_field('add','consultant_information'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['consultant_information']"
                  :localdata="list_user_consultant_information"
              ...
              <uni-forms-item v-if="$check_field('get','name_of_consultant') || $check_field('add','name_of_consultant') || $check_field('set','name_of_consultant')" label="咨询师姓名" name="name_of_consultant">
                                <uni-easyinput type="text" v-model="form['name_of_consultant']" v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','name_of_consultant')) || (!form['complaint_information_id'] && $check_field('add','name_of_consultant'))" :disabled="disabledObj['name_of_consultant_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','time_of_appointment') || $check_field('add','time_of_appointment') || $check_field('set','time_of_appointment')" label="预约时间" name="time_of_appointment">
                        <uni-datetime-picker v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','time_of_appointment')) || (!form['complaint_information_id'] && $check_field('add','time_of_appointment'))" @change="changeLog($event,'time_of_appointment')" v-model="form['time_of_appointment']" type="datetime" :disabled="disabledObj['time_of_appointment_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','time_of_appointment')">
                  {{ form['time_of_appointment'] }}
                </text>
                    </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','reasons_for_complaint') || $check_field('add','reasons_for_complaint') || $check_field('set','reasons_for_complaint')" label="投诉原因" name="reasons_for_complaint">
                        <uni-easyinput type="textarea" v-model="form['reasons_for_complaint']" v-if="user_group === '管理员' || (form['complaint_information_id'] && $check_field('set','reasons_for_complaint')) || (!form['complaint_information_id'] && $check_field('add','reasons_for_complaint'))" :disabled="disabledObj['reasons_for_complaint_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','reasons_for_complaint')">
                  {{ form['reasons_for_complaint'] }}
                </text>
                    </uni-forms-item>
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/complaint_information/upload?'),
