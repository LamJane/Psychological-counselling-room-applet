<template>
	<view class="diy_details diy_div_complaint_information">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','reasons_for_complaint')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>投诉原因:</span>
						</view>
							<view class="diy_field diy_desc">
							<text>
								{{ obj["reasons_for_complaint"] }}
							</text>
						</view>
						</view>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/complaint_information/get_obj", {
					complaint_information_id:this.query.complaint_information_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["complaint_information_id"] !==0) {
...
<style scoped>
	.div_complaint_information_details {}
